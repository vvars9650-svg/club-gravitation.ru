...
assert.match(fs.readFileSync(require.resolve('../admin/index.html'), 'utf8'), /Профиль или мессенджер/);
assert.doesNotMatch(fs.readFileSync(require.resolve('../admin/index.html'), 'utf8'), />Telegram</);
...
(async () => {
  ...
  const client = admin.createClient({__V5_ADMIN_API_URL__: 'https://admin.test/'}, async (url, options = {}) => { calls.push({url, options}); return {status: 200, ok: true, json: async () => ({environment: 'TEST', applications: []})}; }, () => idToken, (reason) => { authFailure = reason; });
  ...
  assert.match(adminSource, /OIDC вход подтверждён\. Admin API ещё не опубликован\./);
