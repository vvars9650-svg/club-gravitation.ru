...
const result = {};
try {
  // Live desktop integration: verify that the production page wires the correct
  // Apps Script URL into the lazy iframe. Google blocks automated cross-origin
  // iframe DOM access, so the full form is exercised top-level immediately after.
  ...
  const iframe = desktop.locator('#application-frame');
  await iframe.waitFor({ state: 'visible', timeout: 15000 });
  const formUrl = await iframe.getAttribute('src');
  if (!formUrl || !/^https:\/\/script\.google\.com\/macros\/s\//.test(formUrl)) {
    throw new Error(`DESKTOP: live site did not wire Apps Script iframe, src=${formUrl}`);
  }

  const desktopForm = await browser.newPage({ viewport: { width: 1440, height: 1000 } });
  await desktopForm.goto(formUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await desktopForm.locator('#application-form').waitFor({ state: 'visible', timeout: 60000 });
  result.desktop = await fillAndSubmit(desktopForm, 'DESKTOP');
  await desktopForm.close();
  await desktop.close();

  // Android-like Chromium: verify closed/open navigation and the actual direct-form fallback.
