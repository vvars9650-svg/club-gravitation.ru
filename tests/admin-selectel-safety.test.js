...
assert.doesNotMatch(adminBundle, /environment:\s*'PROD'|admin-config\.prod\.js|client_secret|refresh_token|-----BEGIN (?:OPENSSH |RSA )?PRIVATE KEY-----/iu);
...
assert.equal(publicFiles.some((file) => /^admin\/(?:.*admin-config|.*admin-auth)/iu.test(file)), false);
assert.doesNotMatch(read(PUBLIC_OUTPUT, 'admin/index.html'), /__V5_ADMIN_CONFIG__|admin:read|admin:write/iu);
assert.match(read(PUBLIC_OUTPUT, 'admin-prod/assets/js/admin-config.prod.js'), /environment: 'PROD'/u);
