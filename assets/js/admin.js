(function bootstrap(root, factory) {
  ...
  function mount(root) {
    ...
    const openCard = async (id) => { ui.drawer.hidden = false; ui.card.textContent = 'Загрузка карточки…'; try { const body = await client.card(id); if (body.environment !== 'TEST') throw new ApiError('environment'); const p = body.participant, application = body.applications[0] || {form:{}}, consents = body.consents || []; const consentSummary = consents.length ? consents.map((item) => `Согласие: ${item.granted === true ? 'получено' : 'не получено'} · Дата: ${esc(item.granted_at)} · Версия согласия: ${esc(item.consent_version)} · Политика: ${esc(item.policy_version)} · Анкета: ${esc(item.form_version)}`).join('<br>') : '—'; ui.card.innerHTML = `<h2>${esc(p.full_name)}</h2>${section('Контакты', ['phone','telegram','email','preferred_contact','public_profile_url','city'].map((key) => [key,p[key]]))}${section('Анкета', FIELDS.map((key) => [key,application.form[key]]))}<section class="card-section"><h3>Работа с заявкой</h3><form class="operational" id="operational-form">${OPERATIONAL.map((key) => key === 'lifecycle_status' ? `<label>${esc(labels[key])}<select name="${key}">${STATUSES.map((item) => `<option ${p[key] === item ? 'selected' : ''}>${esc(item)}</option>`).join('')}</select></label>` : key === 'internal_comment' ? `<label>${esc(labels[key])}<textarea name="${key}">${p[key] ? esc(p[key]) : ''}</textarea></label>` : `<label>${esc(labels[key])}<input name="${key}" value="${p[key] ? esc(p[key]) : ''}"></label>`).join('')}<div class="save-row"><button>Сохранить изменения</button><span id="save-state"></span></div></form></section><section class="card-section"><h3>История заявок</h3>${(body.applications || []).map((item) => `<article class="history-item"><strong>${item.application_number ? `Заявка № ${esc(item.application_number)}` : 'Заявка'}</strong><br>${esc(item.submitted_at)}</article>`).join('') || '—'}</section><section class="card-section technical"><h3>Сведения о согласии</h3>${consentSummary}</section>`; const form = doc.querySelector('#operational-form'); form.onsubmit = async (event) => { event.preventDefault(); const saveState = doc.querySelector('#save-state'); saveState.textContent = 'Сохраняем…'; try { await client.save(id, Object.fromEntries(new FormData(form))); saveState.textContent = 'Сохранено'; load(); } catch (error) { saveState.textContent = error.code === 'access_denied' ? 'У вас нет прав для этого действия.' : 'Не удалось сохранить изменения. Попробуйте позже.'; } }; } catch { ui.card.textContent = 'Не удалось загрузить карточку. Попробуйте позже.'; } };
