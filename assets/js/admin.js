(function bootstrap(root, factory) {
  ...
  function diagnosticText(label, diagnostic) {
    const values = [`JWT: ${diagnostic.is_jwt ? 'да' : 'нет'}`, `segments: ${diagnostic.segment_count}`, `iss: ${diagnostic.iss || '—'}`, `aud: ${Array.isArray(diagnostic.aud) ? diagnostic.aud.join(', ') : diagnostic.aud || '—'}`, `sub present: ${diagnostic.sub_present ? 'да' : 'нет'}`, `exp: ${diagnostic.exp || '—'}`, diagnostic.scope !== undefined ? `scope: ${diagnostic.scope || '—'}` : null, `claims: ${diagnostic.claim_names.join(', ') || '—'}`].filter(Boolean);
    return `${label}\n${values.join('\n')}`;
  }
  function mount(root) {
    ...
    if (!list) return;
    const ui = {auth: doc.querySelector('#admin-auth'), app: doc.querySelector('#admin-workspace'), identity: doc.querySelector('#admin-identity'), diagnostic: doc.querySelector('#admin-token-diagnostic'), login: doc.querySelector('#admin-login'), logout: doc.querySelector('#admin-logout'), state: doc.querySelector('#admin-state'), search: doc.querySelector('#admin-search'), status: doc.querySelector('#admin-status'), owner: doc.querySelector('#admin-owner'), priority: doc.querySelector('#admin-priority'), sort: doc.querySelector('#admin-sort'), drawer: doc.querySelector('#participant-drawer'), card: doc.querySelector('#participant-card')};
