...
from backend.v5.tests.test_v5 import P, e
...
class PhaseBParticipantTests(unittest.TestCase):
    ...
    def test_new_application_creates_canonical_participant_and_phone_key(self):
        repo = FakeRepository()
        record, replay = submit(P, "phase-b-new", repo, "request-new")
    ...
    def test_second_application_reuses_participant_without_overwrite(self):
        repo = FakeRepository()
        first, _ = submit(P, "phase-b-first", repo, "request-first")
        ...
        second, _ = submit(changed, "phase-b-second", repo, "request-second")
    ...
    def test_participant_admin_change_does_not_mutate_application(self):
        repo = FakeRepository()
        record, _ = submit(P, "phase-b-admin", repo, "request")
    ...
    def test_idempotent_replay_and_conflicting_replay(self):
        repo = FakeRepository()
        first, replay = submit(P, "phase-b-idempotent", repo, "request-1")
        second, replay_second = submit(P, "phase-b-idempotent", repo, "request-2")
        self.assertFalse(replay)
        ...
        with self.assertRaises(DomainError) as raised:
            submit({**P, "full_name": "Changed"}, "phase-b-idempotent", repo, "request-3")
    ...
    def test_broken_phone_key_fails_without_reassignment(self):
        ...
        response = handler(e(P, "phase-b-broken"), repo=repo)
